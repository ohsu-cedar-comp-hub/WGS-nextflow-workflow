...
This Nextflow workflow aligns, calls, and annotates whole genome sequencing data using Singularity. Submodules of the workflow are described and can be run independently. Submodules of the workflow are described below and can be run independently assuming the necessary input files exist. A parameter file is passed as input using -params-file <my-params.json>, which can be generated using the templating script.
